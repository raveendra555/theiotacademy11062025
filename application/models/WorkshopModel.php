...
class WorkshopModel extends CI_Model
{
    ...
    // ===============================
    // Get rows with dynamic conditions
    // ===============================
    public function getRows($params = array()) {
        $this->db->select('*');
        $this->db->from('workshop_certificate');
        ...
        if (array_key_exists("returnType", $params) && $params['returnType'] == 'count') {
            ...
            if (array_key_exists("id", $params)) {
                ...
                $query = $this->db->get(); // ✅ Removed duplicate table name
    ...
    public function get_data() {
        $query = $this->db->query("SELECT * FROM workshop_certificate WHERE status = 1 ORDER BY id DESC");
    ...
    public function fetch_certificate_name($offerid) {
        ...
        $data = $this->db->get('workshop_certificate');
...
<body class="fstdnnn" style="padding-bottom:30px;z-index:9999;">
   <div>
        ...
        <div style="padding-top:20px;text-align:center;"> <span style="letter-spacing:3px;font-size:65px;font-family: "Times New Roman", Times, serif; font-weight:bold;color:rgb(56, 52, 52);">Certificate of Participation</span> <br><div style="font-size:30px; padding-top:8px;color:rgb(52, 49, 49);">This is to certify that</div> <br>
          ...
          <div style="line-height: 28px;margin-top:14px;"> <span style="font-size:25px;color:rgb(52, 49, 49);">from <b>'.$row->college_name.'</b> has successfully participated in the </span> </div><br> 
          <div><p style="font-size:25px;color:  rgb(56, 52, 52);margin:0;padding:0;"><b>'.$row->topic.'</b></p>
              <p style="font-size:20px;color: rgb(52, 49, 49);margin-top:14px;padding:0;">workshop conducted by The IoT Academy and Uniconverge Technologies.</p></div>
    ...
    // Helper to base64 encode images safely
    private function encode_image($path) {
        $full_path = FCPATH . str_replace(base_url(), '', $path);
    ...
    public function user_data() {
        $query = $this->db->query("SELECT * FROM workshop_certificate WHERE status = 0");
        return ($query->num_rows() > 0) ? $query->result_array() : [];
    }
    ...
    public function insert($data = array()) {
        if (!empty($data)) {
            ...
            $insert = $this->db->insert("workshop_certificate", $data);
    ...
    public function update($data, $condition = array()) {
        if (!empty($data)) {
            ...
            return $this->db->update("workshop_certificate", $data, $condition);
        }
        return false;
    }
    ...
    public function fetch_member_email($Ids) {
        $query = $this->db->query("SELECT * FROM `workshop_certificate` WHERE `id` IN ($Ids) LIMIT 30");
        return ($query->num_rows() > 0) ? $query->result() : false;
    }

    public function user_mail_status($ID) {
        return $this->db->query("UPDATE `workshop_certificate` SET `mail_status`=1 WHERE `id` = '$ID'");
    }
}
?>
