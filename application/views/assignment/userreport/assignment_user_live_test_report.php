    ...
    <body>
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-9 col-lg-10">
                <div class="container">
                     <h3 class="mt-4 mb-4" style="font-size: 20px;">User Mini Test1/Mini Test2/Live Test/Assignment Report</h3>
    <!-- Bar Chart -->
    <div class="card-custom">
        <h4 class="text-center mb-4">All Topics - Test Performance</h4>
        <canvas id="barChart"  style="background: none; border: none;"></canvas>
    </div>    
        ...
        <table id="member_data" class="table table-striped table-bordered">
            <thead class="thead-dark">
                <tr>
                    ...
                    <th class="text-center">Mini Test1</th>
                    <th class="text-center">Mini Test2</th>
                    <th class="text-center">Live Test</th>
                ...
                {   $no_id=1;
                    ...
                    { 
                        ...
                        <tr>
                            ...
                            <td class="text-center"><?php echo $row['mini_test1']; ?></td>
                            <td class="text-center"><?php echo $row['mini_test2']; ?></td>
                            <td class="text-center"><?php echo $row['live_test']; ?></td>
...
if (!empty($members)) {
    foreach ($members as $row) {
        ...
        $mini_test1[] = round(($row['mini_test1'] / 10) * 100, 2);
        $mini_test2[] = round(($row['mini_test2'] / 10) * 100, 2);
        $live_test[] = round(($row['live_test'] / 15) * 100, 2);
        $assignment[] = round(($row['assignment'] / 20) * 100, 2);
    }
}
?>
<script>
  const topicLabels = <?= json_encode(array_map(fn($t, $d) => "$t ($d)", $topic_name, $date_range)) ?>;
  const dateRanges = <?= json_encode($date_range) ?>;
  const miniTest1 = <?= json_encode($mini_test1) ?>;
  const miniTest2 = <?= json_encode($mini_test2) ?>;
  const liveTest = <?= json_encode($live_test) ?>;
  ...
  // Pie Charts
  const topicCount = <?= count($topic_name) ?>;
  const pieData = {
    miniTest1: <?= json_encode($mini_test1) ?>,
    miniTest2: <?= json_encode($mini_test2) ?>,
    liveTest: <?= json_encode($live_test) ?>,
